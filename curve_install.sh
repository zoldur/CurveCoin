...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='curve.conf'
CONFIGFOLDER='/root/.curvecore'
COIN_DAEMON='/usr/local/bin/curved'
COIN_CLI='/usr/local/bin/curve-cli'
...
function compile_node() {
  ...
  wget -q $COIN_REPO 
  compile_error 
  unzip curvecoin-wallet-ubuntu16.04.zip >/dev/null 2>&1
  compile_error 
  rm curvecoin-wallet-ubuntu16.04.zip >/dev/null 2>&1
  chmod +x curve* >/dev/null 2>&1
  cp curve* /usr/local/bin
  cd -
...
function enable_firewall() {
  echo -e "Install and configure ufw firewall to allow ingress on port ${GREEN}$COIN_PORT${NC}"
